...
def rolling_plot_time_mean(file_path_rolling):
    ...
    abs_window_mean = df_abs_window.mean(axis = 0,numeric_only=True)

    return abs_window_mean
    # print(df_1_year_window_data[-1])
    # print(df_abs_window_mean)
    
    # plt.plot(abs_window_mean)

    # plt.show()
    

def clifton_ward_corr_plot(file_path_rolling):
    """
    Plot how the correlation between Clifton and all the other wards changes over time    
    """

    ward_corr_data = pd.read_csv(file_path_rolling)

    # Extract ward correlation data between Clifton and all the other wards
    Clifton_corr_data = ward_corr_data[ward_corr_data['ward i'] == 'Clifton']
    Clifton_corr_data = Clifton_corr_data.iloc[:,1:]
    Clifton_corr_data= Clifton_corr_data.T
    header = Clifton_corr_data.iloc[0]
    Clifton_corr_data = Clifton_corr_data[1:]
    Clifton_corr_data.columns = header

    plot = Clifton_corr_data.plot(linewidth = 1, ylim=[-1,1]) # , linestyle="dashdot")

    # plt.legend(fontsize=10)

    plt.show() 
    # return Clifton_corr_data
...
def main():
    ...
    # Generate heatmap showing best lags
    lag_matrix_heatmap(lag_matrix,unique_wards)
    

    # Calculate rolling correlations between ward pairs for different window sizes
    ...
    rolling_data_1_year_window = rolling_plot_time_mean(file_path_rolling_4)
    rolling_data_2_year_window = rolling_plot_time_mean(file_path_rolling_8)
    rolling_data_3_year_window = rolling_plot_time_mean(file_path_rolling_12)
    rolling_data_5_year_window = rolling_plot_time_mean(file_path_rolling_20)
    rolling_data_10_year_window = rolling_plot_time_mean(file_path_rolling_40)

    fig, ax = plt.subplots()
    ax.plot(rolling_data_1_year_window, label = "1-Year Window")
    ax.plot(rolling_data_2_year_window, label = "2-Year Window")
    ax.plot(rolling_data_3_year_window, label = "3-Year Window")
    ax.plot(rolling_data_5_year_window, label = "5-Year Window")
    ax.plot(rolling_data_10_year_window, label = "10-Year Window")

    ax.set_ylim([0,1])
    ax.set_xlabel("Year")
    ax.set_ylabel("Mean absolute ward-pair correlation coefficient") # Pearson correlation coefficient
    
    ax.legend()

    plt.show()
     
    clifton_ward_corr_plot(file_path_rolling_4)
    clifton_ward_corr_plot(file_path_rolling_8)
    clifton_ward_corr_plot(file_path_rolling_12)
    clifton_ward_corr_plot(file_path_rolling_20)
    clifton_ward_corr_plot(file_path_rolling_40)
